...
export function CourseGrid() {
  ...
  return (
    <div>
          ...
          return (
            <button
              ...
              className={`relative cursor-pointer rounded-full px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.16em] transition-colors duration-500 ${
                isActive ? "text-cream" : "text-mist hover:text-ink"
              }`}
            >
              {isActive && (
                <motion.span
      ...
      {filtered.length === 0 && (
        <div className="mt-12 rounded-[14px] border border-dashed border-ink/20 px-8 py-16 text-center">
          <p className="font-serif text-2xl italic text-mist">
